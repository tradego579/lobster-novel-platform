import { PrismaClient, Category, NovelStatus, Role, ChapterStatus } from '@prisma/client'
...
async function main() {
  ...
  // 创建章节
  const createChapters = async (novelId: string, titles: string[]) => {
  ...
  await prisma.readingHistory.create({
    data: {
      userId: adminUser.id,
      novelId: novel1.id,
      chapterId: (await prisma.chapter.findFirst({ where: { novelId: novel1.id } }))!.id,
    },
  })
